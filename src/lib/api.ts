...
export async function fetchHealth() {
  const res = await fetch(`${getApiBase()}/api/health/detailed`);
...
export async function fetchNodes() {
  const res = await fetch(`${getApiBase()}/api/nodes`);
...
export async function fetchNode(nodeId: string) {
  const res = await fetch(`${getApiBase()}/api/nodes/${nodeId}`);
  if (!res.ok) throw new Error('Failed to fetch node');
  return res.json();
}

export async function fetchNodeEvents(nodeId: string, limit = 50) {
  const res = await fetch(`${getApiBase()}/api/nodes/${nodeId}/events?limit=${limit}`);
...
export async function fetchEvents(limit = 50) {
  const res = await fetch(`${getApiBase()}/api/events?limit=${limit}`);
